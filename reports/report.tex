...
Als Ziel-Edge-Hardware wird ein Einplatinencomputer wie der Raspberry Pi~4 (1,5~GHz
ARM Cortex-A72, 2--4~GB RAM) angenommen. Die Sensordaten werden stündlich als
vier Messwerte (PM2.5, PM10, NO2, CO) bereitgestellt. Die Anforderungen an das
Edge-Modell ergeben sich daraus: geringe Modellgröße (wenige KB), kurze Inferenzzeit
(unter 100~ms) und keine Abhängigkeit von einer Internetverbindung für die Vorhersage.
...
\parencite{devito2016uci} (globale Städte), das Umweltbundesamt
...
Der zweite Schritt erzeugt elf zeitbasierte Merkmale. Neben den Rohwerten für Stunde,
...
Nach Abschluss aller fünf Schritte umfasst der Datensatz die vier Rohwerte sowie 24
engineerte Merkmale, insgesamt also 28~Spalten. Durch die Lag-, Rolling- und
Differenzberechnungen gehen die ersten 29~Datenpunkte verloren, sodass der finale
Datensatz rund 8\,520 stündliche Beobachtungen enthält.
...
zukünftigen Daten evaluiert wird. Die ersten 80\,\% der Beobachtungen (ca. 6\,820 Stunden)
bilden den Trainingssatz, die letzten 20\,\% (ca. 1\,700 Stunden) den Testsatz.
...
gesamten Testsatz (ca. 1\,700 Datenpunkte) unter 0,1~ms. Diese Werte liegen weit unter den
...
Das Ziel des Projekts war die Entwicklung eines Edge-AI-Systems zur Vorhersage der
Luftqualität, das bei erhöhter Schadstoffbelastung automatisch Verkehrsmaßnahmen auslösen
kann. Dieses Ziel wurde erreicht. Die vollständige Pipeline umfasst den Datenabruf über
die OpenAQ-API, die Vorverarbeitung mit Feature Engineering, das Training und den Vergleich
von vier Modelltypen für beide Zielvariablen, die Feature-Reduktion von 28 auf 17~Merkmale,
den \ac{ONNX}-Export sowie ein kombiniertes Ampelsystem mit Edge-Inference-Demo.

Die lineare Regression erzielte für PM2.5 einen \ac{MAE} von 0,914\,\textmu g/m\textsuperscript{3}
und ein $R^2$ von 0,931, für NO2 einen \ac{MAE} von 4,199\,\textmu g/m\textsuperscript{3}
und ein $R^2$ von 0,871. Trotz ihrer Einfachheit übertrifft sie die komplexeren Modelle
und wurde daher als Edge-Modell gewählt. Die exportierten \ac{ONNX}-Modelle belegen jeweils
nur 0,3~KB und benötigen unter 0,02~ms pro Vorhersage, womit sie für den Einsatz auf
ressourcenbeschränkten Geräten geeignet sind.

\subsection{Kritische Reflexion}
Das Projekt weist mehrere Einschränkungen auf, die bei der Interpretation der Ergebnisse
berücksichtigt werden sollten. Die Trainingsdaten stammen von einer einzigen Messstation
und decken lediglich ein Kalenderjahr ab. Ob die Modelle auf andere Standorte oder Jahre
übertragbar sind, wurde nicht untersucht.

Die Feature-Reduktion verwendet für beide Schadstoffe dasselbe reduzierte Set von
17~Merkmalen. Während dies den Edge-Einsatz vereinfacht, führt es bei NO2 zu einer
messbaren Verschlechterung, da tageszeitliche Features hier relevanter sind als bei PM2.5.
Ein schadstoffspezifisches Feature-Set könnte die NO2-Vorhersage verbessern, würde jedoch
die Feature-Berechnung auf dem Edge-Gerät komplexer machen.

Die Schwellenwerte des Ampelsystems wurden an den WHO-Richtwerten orientiert, jedoch
nicht empirisch auf die lokalen Gegebenheiten an der Habichtstraße kalibriert. In der
Praxis müssten die Schwellenwerte in Abstimmung mit den zuständigen Behörden festgelegt
und validiert werden.

Eine weitere Herausforderung im produktiven Edge-Betrieb ist Data Drift: Die
Schadstoffverteilungen können sich durch Veränderungen im Verkehrsaufkommen, Baustellen
oder saisonale Effekte über die Zeit verschieben. Das Modell wurde auf Daten eines
einzelnen Jahres trainiert und könnte bei veränderten Bedingungen an Vorhersagequalität
verlieren. Ein regelmäßiges Monitoring der Vorhersagefehler und ein Mechanismus zum
Nachtrainieren wären im Dauerbetrieb notwendig. Zudem gelten für Edge-Geräte
spezifische Restriktionen wie begrenzter Speicher, eingeschränkte Stromversorgung und
fehlende permanente Internetverbindung, die bei einem Deployment berücksichtigt werden
müssen.

\subsection{Verbesserungspotenziale}
Mehrere Ansätze könnten die Vorhersagequalität und den praktischen Nutzen des Systems
verbessern. Die Integration externer Datenquellen wie Wetterdaten (Windgeschwindigkeit,
Temperatur, Niederschlag) oder Verkehrszähldaten würde den Modellen zusätzliche
Erklärungsvariablen liefern, die den Schadstoffausstoß direkt beeinflussen.

Auf Modellseite könnte eine systematische Hyperparameter-Optimierung mittels Grid Search
oder Bayesian Optimization die Ergebnisse der baumbasierten Modelle und des \ac{GRU}-Netzwerks
verbessern. Auch die Erprobung weiterer Architekturen wie LSTM oder Transformer-basierter
Zeitreihenmodelle wäre denkbar.

Für den Edge-Einsatz könnte eine Quantisierung der \ac{ONNX}-Modelle (z.\,B. INT8) die
Inferenzzeit und den Speicherbedarf weiter reduzieren. Zudem wäre eine Validierung auf
realer Edge-Hardware wie einem Raspberry Pi oder einem Mikrocontroller sinnvoll, um die
Praxistauglichkeit unter realen Bedingungen zu bestätigen.

\subsection{Ausblick}
Das entwickelte System bildet eine solide Grundlage für weiterführende Arbeiten im Bereich
der Edge-basierten Umweltüberwachung. Ein nächster Schritt wäre die Erprobung im
Feldversuch, bei der das System an einer realen Messstation installiert und über mehrere
Monate betrieben wird. Dabei könnten die Vorhersagen kontinuierlich mit den tatsächlichen
Messwerten verglichen und die Modelle bei Bedarf nachtrainiert werden.

Langfristig ließe sich das Konzept auf ein Netzwerk mehrerer Stationen erweitern, sodass
stadtweit ein flächendeckendes Frühwarnsystem entsteht. Durch die Kombination der
Vorhersagen benachbarter Stationen könnten räumliche Ausbreitungsmuster erkannt und
Maßnahmen gezielter koordiniert werden.
